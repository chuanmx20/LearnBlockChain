...
BigInt BigInt::operator<<(int lsc) const {
    return *this * pow(2, lsc);
}

BigInt BigInt::operator>>(int rsc) const {
    return *this / pow(2, rsc);
}

BigInt &BigInt::operator<<=(const int &lsc) {
    *this *= pow(2, lsc);
    return *this;
}

BigInt &BigInt::operator>>=(const int &rsc) {
    *this /= pow(2, rsc);
    return *this;
}
